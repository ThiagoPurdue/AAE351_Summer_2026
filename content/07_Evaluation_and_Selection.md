# Evaluation and Selection

## Core Principles of Evaluation and Selection

# 07 Evaluation and Selection

Evaluation and Selection
AAE35103 Evaluating and selecting the system 
concept
2
Kossiakoff et al., Systems Engineering Principles and Practice, Third ed., Wiley, 2020 
“What are the key characteristics of a system concept that would 
achieve the most beneficial balance between capability, 
operational life, and cost?”
To answer this question:
DONE
DURING
THE
CONCEPT DEFINITION
PHASE
· A
number
of
alternatives
concepts
should
be
developed
· EVALUATE them regarding
:
(n)
OPERATIONAL
PERFORMANCE
AND
compatibility
(ii) Risks
in
achieving
each of the
above 3
READY
TO
Make A
Liri) Prostam cost
and
schedule
Decision Evaluating and selecting the system 
concept
3
Kossiakoff et al., Systems Engineering Principles and Practice, Third ed., Wiley, 2020 
• Program risk can be considered to consist of a 
combination of two factors:
•
Program risks can result from several sources:
-
> Likelihood the system
will
fail
to
achieve this objective
-
>Import
of
the
failure
on the
success
of the program
↓
UMPROVEN
TECHNOWGY
-
DiFICULT
PERFORMANCE
REQUIREMENTS
+
SEVERE
Enviroment
-
INADEQUAE
FUNDING
*
MORE
ON
RiSk
+
SMORT
SCHEDULES
IN
A
DEDICATED
LECTURE Evaluating and selecting the system 
concept
4
Kossiakoff et al., Systems Engineering Principles and Practice, Third ed., Wiley, 2020 
Justification for the development of the selected concept should:
o  
o  
o  
o  
o  
o  
o  
Show
there
is
a
valid
need
to
be
met
State
reasons for selecting
a
concept
Describe program risks
X mitigation
Evidence
of previous
experience& success
Evidence
of
detailed plans
,
e. g., work Breakdown structure
(Was)
rife
Cycle Costing
OTHER
issues
,
e . g., environment impact Business decisions are not simple
5
• Which engineering projects are worthwhile?
o Has the space engineer shown that the space system is worth developing? 
• Which engineering projects should have a higher priority?
o Has the aeronautical engineer shown which aircraft improvement projects should be funded 
with the available dollars?
• How should the engineering project be designed?
o Has the mechanical engineer chosen the best thickness for insulation?
• All decisions are interrelated.
o You can’t make one without affecting many others
o Especially true when multiple products, periods, and stages are involved
Miller et al 2004 Why cost estimation?
6
• Critically important part of system design
o  
o  
• Fundamental estimation questions:
o How much effort is required to complete an activity?
o How much calendar time is needed to complete an activity?
o What is the total cost of an activity?
o What are the uncertainties in these estimates?
• Project estimation and scheduling are interleaved management activities
Miller et al 2004
TOO
HIGH
=
lose
contract
, award ! customer
Too
low
=
over-run costs/company
loss
S
total working hours Cost estimation process
7
• Determine objectives
o Who needs what data for what purpose(s)?
• Gather data
o Focus should be given to ‘hard’ data
§ Well-defined requirements
§ Available Resources
• Analyze data using a variety of methods
• Re-estimate costs throughout the project
o Effective monitoring
o Refine and make changes as necessary
• Compare end costs with estimated costs
o That is, if project actually completes!
Prakash, Grewal and Chen, 2008 Cost estimation process
8
NASA Cost Estimating Handbook, v4.0
• Calibrate estimates based on actual 
costs
• Re-estimate at the end of each phase 
and iteration.
• Use estimates to identify productivity 
improvement opportunities.
“A good estimate is an estimate that provides a clear enough view of the project 
reality to allow the project leadership to make good decisions about how to control 
the project to hit its targets”
Steve McConnell
 
Figure C-1. Use of Cost Estimating Methodologies by Phase1 Cost estimation methods
9
Analogy
• Data-driven
• Based on similarity
• Can be applied at any level of 
design
• Pros: 
o Quick rough order magnitude (ROM) 
estimate with a few known 
characteristics
• Cons: 
o Getting good data might be difficult
o Analogy data might not be available 
because of new systems' uniqueness
o Inflexible for trade studies
Parametric
•
Data-driven
•
Statistical relationship model 
based on historic actual 
between costs and a system or 
performance characteristics
•
Typical parametric cost model 
are based on mass and power
•
Best for trade studies during the 
conceptual design phase
•
Pros:
o
Provides estimate confidence based 
on actual data and statistical 
relationship
•
Cons:
o
Questionable when modeling 
outside of its relevant data range
Grassroots
•
Data-driven
•
Also known as “bottoms-up”
•
Experienced and/or knowledge 
from subject matter expert 
required
•
Applied after an architecture 
has been selected and the 
design is mature
•
Pros:
o
Most accurate
o
Defensible with detailed and 
credible basis of estimate
•
Cons:
o
Most time consuming
o
Not suitable for a quick ROM
~
& Analogy cost estimation example
10
•
Estimate the cost of a spacecraft by analogy method
•
Use mass as the characteristic
•
Have data based on historic missions
•
Average spacecraft mass = ~ 150 kg and cost = ~ $50M
•
A new spacecraft which is similar to the ones in the data set has a mass = ~200 kg
•
Then, by similarity we can say
•
So, new the spacecraft cost is estimated as =
-K=
N 67M Cost estimation relationships (CERs)
11
•
A parametric cost model is a series of mathematical relationships that relate a 
dependent variable (cost) to one or more independent variables 
o
E.g., spacecraft cost to physical, technical, and performance parameters
o
E.g., aircraft cost to mass
•
Based on specific factors that have a high correlation to total cost
o
Number of software lines of code (SLOC) or function points
o
Square feet for office floor space
o
Number of floors in a high rise building for cabling estimates
o
Database size, etc.
•
Based on regression analysis & the correlation of data
o
Requires a historical database
o
  
•
To be used as a comparison tool, not a budgeting tool!
-
mass
Cost
=
a + bM'pd
a ,b ,a, d
- Power
REGRESSION
DEFFS . Cost estimation relationships (CERs)
12
mass
cost
S
Cost
of
RANGE of
PREDICTED -p
& VALID In THE
-
DATA
YOUR
NEW
Linear
SYSTEM
/GETTHE
BEST FIT MODELI(
I data
1
ofmartem Example of CER use
13
• Historical data has enabled the organization to have a previously developed 
CER to estimate a new IT system based on SLOC
• Cost = SLOC * 25 $/SLOC
• The CER is based on systems ranging from 1,000,000 to 3,000,000 SLOC
• You have estimated 2,600,000 SLOC for a new system
• à Cost =
~
Source
Lines
Of
CODE
-
historical data = Measure size of A computer program
- E
. JUDGEMENT
2, 600 , 000
x
$25 =$65M RDT&E and production CERs for large 
satellites
14
Miller et al 2004
I
-1992
RESEARCH
~dollars
DEVELOPMENT
TEST
AND
ENGINEERING Higher-level cost factors
15
Miller et al 2004
• Wraps model non-physical factors
o Program support, system engineering, management costs, product assurance, integration 
and test
• Overhead is incurred in support of an activity, but is not solely identifiable to that 
activity
o Administration, real estate taxes, facility maintenance
• Program support
o Activities, systems and hardware development outside the prime contract
§ Analyses
§ Government test facilities, equipment and personnel
§ Program office support contractors for technical oversight
o Advanced development studies Management reserves
16
Miller et al 2004
• Account for cost and schedule uncertainty
o Complexity factor uncertainty
o Cost driver input uncertainty
o Time and material cost
• Generally identified/quantified at the subsystem level
o Basis
o Heritage
o Degree of modification
o Complexity
o Is the item on the critical path?
o Is new development or technology required?
o Past performance
o Are long-lead procurements involved?
o Is an iterative test and retest program required? 
o Availability of people and facilities
• Reserves should be held at management level
• Should not be used to accommodate scope changes Impact of technological risk on cost
17
Miller et al 2004
TRL Pros and cons of parametric models
18
• Pros:
o Can be excellent predictors when implemented correctly
o Once created, CERs are fast and simple to use
o Easily changed
o Useful early on in a program
o Objective
• Cons:
o Often lack of data on software intensive systems for statistically significant CER
o Does not provide access to subtle changes
o Top level; lower level may be not visible
o Need to be properly validated and relevant to system
GAO, 2003
· must also determine range
of date
for
which
the
CER
is
valid
. Engineering/grassroot build-up cost 
estimates
19
 
Figure C-10.  Method for Developing Engineering Build-Up Estimate 
Start 
Segregate Into 
CBS/WBS 
Test for 
Omissions and 
Duplications 
Aggregate 
“Work Packages” 
Estimate 
Individual  
“Work Packages” 
Decompose 
CBS/WBS into 
“Work Packages” 
Perform  
“Sanity Check” 
Aggregate Into 
“Total Estimate” 
NASA Cost Estimating Handbook v4.0
we Work breakdown structure (WBS) 
example
20
Project Mgmt 
01 
Project Sys Eng'g 
02 
Mission Assurance  
03 
Science 
04 
Atmospheric 
Flight System 
05 
S/C Flt System 
06 
Mission Ops Sys 
07 
Launch System 
08 
Project Management 
01.01 
Business Mgmt 
01.02 
Project Support 
01.03 
Review Support 
01.04 
Facilities 
01.05 
Foreign Travel 
01.06 
Tech Xfer/Comrcl. 
01.07 
Project Sys. Eng Mgmt 
02.01 
Mission & Nav Design 
02.02 
Risk Management 
02.03 
Project S/W Eng'g 
02.04 
Info. Mgt. & Comm. 
02.05 
Configuration Mgmt. 
02.06 
Planetary Protection 
02.07 
NEPA 
02.08 
E-E Info System 
02.09 
Launch Vehicle Integ. 
02.10 
Project V&V 
02.11 
Mission Assur. Mgmt. 
03.01 
System Safety 
03.02 
Reliability 
03.03 
Environments 
03.04 
Quality Assurance 
03.05 
EEE Parts Eng'g 
03.06 
Materials & Processes 
03.07 
Contamination Cntl 
03.08 
Mission Ops Assurance 
03.09 
IV&V 
03.10 
Science Mgmt 
04.01 
Science Team 
04.02 
Science Data Supt 
04.03 
EPO 
04.04 
Sci. Inves. & Ops Supt 
04.05 
Science Environmental 
Characterization 
04.06 
Atmos. Flight 
System Mgmt. 
05.01 
Atmos. Flight Sys. 
Systems Engr. 
05.02 
Atmos. Flight Sys. 
Mission Assurance 
05.03 
Atmos. Flight Sys. 
Airplane 
05.04 
Atmos. Flight Sys. 
Instrumentation 
05.05 
Atmos. Flight Sys. 
Risk Reduction 
05.06 
Atmos. Flight Sys. 
Mission Ops. 
05.07 
Flight System Contract 
06.00 
Flt System Mgmt 
06.01 
Flt System Eng'g 
06.02 
Flt System Testbeds 
06.03 
Flight System ATLO 
06.04 
Carrier System 
06.05 
EDD System 
06.06 
Mission Syst. Mgmt 
07.01 
Mission Syst. Eng'g 
07.02 
Ground Data System 
07.03 
Mission Ops System 
07.04 
DSMS 
07.05 
Mission & Nav. Design 
07.06 
Mission Syst. V&V 
07.07 
Launch System 
08.01 Attributes of a good cost estimate
21
• Aligned with integrated master schedule
• Grass-roots estimation performed by cognizant engineers with sound basis of 
estimate
• Consistent with validated historical data
• Key cost drivers and risk are understood
• Validated with several independent cost models
• Well documented work content and basis of estimate
• Developed at lowest level of the work breakdown structure (WBS) Resources
22
• Aircraft
o Aircraft Airframe Cost Estimating Relationships by Hess and Romanoff
§ https://www.rand.org/pubs/reports/R1854.html 
o Aircraft Design: A Conceptual Approach by Raymer
o Fundamentals of Aircraft and Airship Design, Vol. I – Aircraft Design by Nicolai and Carichner
o General Aviation Aircraft Design by Gudmundsson (Ch. 2 Aircraft Cost Analysis posted on 
Brightspace; also available through the Purdue library website)
• Spacecraft
o NASA cost guidance
§ https://www.nasa.gov/offices/ocfo/nasa-cost-estimating-handbook-ceh
§ Lots of interesting information, check it out!
§ Especially Appendix C
o Space mission engineering: the new SMAD by Wertz, Everett, and Puschell Ideas for your project
23
•
Development cost
o
Discuss the types of costs
o
Use any historical data (if available) to obtain rough order of magnitude (ROM) estimates
o
Try to use the CER approach to estimate development cost
o
If no CERs are available
§ Try to discuss what major parameters will be important to an eventual CER model
§ Discuss qualitatively how you would use the CER approach to obtain a cost estimate
•
Operating cost
o
Discuss the types of costs
o
Discuss qualitatively the operating costs
§ Are they more / less than competing systems?
§ Competition includes systems that are fundamentally different, but meet the same need
For
comparison of
CONCEPTS , NOT
BUDGETING Reading
24
Chapter 8 of Kossiakoff et al., Systems Engineering Principles and Practice, 
Third ed., Wiley, 2020
Aircraft Airframe Cost Estimating Relationships 
https://www.rand.org/pubs/reports/R1854.html 
NASA cost guidance
https://www.nasa.gov/offices/ocfo/nasa-cost-estimating-handbook-ceh Reading
24
Chapter 8 of Kossiakoff et al., Systems Engineering Principles and Practice, 
Third ed., Wiley, 2020
Aircraft Airframe Cost Estimating Relationships 
https://www.rand.org/pubs/reports/R1854.html 
NASA cost guidance
https://www.nasa.gov/offices/ocfo/nasa-cost-estimating-handbook-ceh


## System Integration

Using tools like the Pugh Matrix, you will evaluate competing RCUAV architectures. The evaluation criteria must heavily weight Additive Manufacturing constraints—such as print time, required support structures, and assembly complexity—against aerodynamic efficiency and payload capacity.
